...
2. 최종 일관성 (Eventual Consistency) : 읽은 시점 직전의 작업의 결과가 반영되지 않을 수 있는 일관성 (강력한 일관성 (Strong Consistency)과 반대)
<div align="center">
...
-----
1. 초당 4KB(4096 Byte)의 데이터를 읽을 수 있는 티켓
2. 초당 강력한 일관성 읽기 1번(4KB)은 티켓 하나 필요
3. 초당 최종 일관성 읽기 1번(4KB)은 티켓 0.5개 필요 (= 티켓 하나로 최종 일관성 두 번 수행 가능)
4. 초당 트랜잭션 읽기 1번(4KB)은 티켓 두 개 필요

-----
### Write Capacity Unit(WCU)
-----
1. 초당 1KB(1024 Byte)의 데이터를 쓸 수 있는 티켓
2. 초당 일반 쓰기(1KB)는 티켓 하나 필요
3.  초당 트랜잭션 쓰기 1번(1KB)은 티켓 두 개 필요
...
### Burst Capacity
-----
1. 300초(5분)동안 사용하지 않은 RCU와 WCU를 모아서 Brust에 활용
2. 갑자기 많은 양의 요청이 올 때 혹은 큰 읽기 쓰기 작업에 사용
3. 혹은 내부적인 유지 보수에 사용
<div align="center">
...
### 읽기 용량
-----
1. 단위 : 4KB (예) 3.5KB : 4KB로 취급, 10KB는 12KB로 취급)
2. 종류
...
2. 조건 쓰기 : 특정 조건을 만족할 경우에만 쓰기 허용 (예) Count가 1이라면 2로 업데이트)
