...
4. Demo
   - SNS - 버지니아 북부 리전 - 샌드박스 대상 전화번호 - 전화번호 추가 (한국어) - 확인 코드로 추가
   - 문자 메세지 개시 - 프로모션 / 트랜잭션 (트랜잭션 선택) - 대상 번호 / 메세지 입력 - 발신 ID 입력 가능 - 메세지 전송
   - 문자 메세지 기본 설정 : 계정 지출 한도 (월별 $1) - Support Case로 변경해야 함
   - SNS 샌드박스 종료 - Support Case 작성
